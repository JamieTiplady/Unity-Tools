...
namespace JamieTipladyTools.BridgeGenTool 
{
    [ExecuteAlways]
    public class BridgeGenerator : MonoBehaviour
    {
        ...
        private List<GameObject> _editorObjectPool = new List<GameObject>();
        ...
        public void RebuildBridge()
        {
            ...
            //put all existing objects into a pool instead of destroying them
            PopulatePool();

            _contentContainer = transform.Find("[Bridge Gen Content]");
            if (_contentContainer == null)
            {
                GameObject containerObj = new GameObject("[Bridge Gen Content]");
                _contentContainer = containerObj.transform;
                _contentContainer.SetParent(transform, false);
            }

            SplineContainer container = SplineContainer;
            if (container == null)
            {
                TrimPool(); //clean up unused pooled items if spline is null
                return;
            }
            ...
            {
                TrimPool(); //clean up unused pooled objects if spline is shorter
                return;
            }
            ...
            //Destroy any objects left in the pool that weren't needed for the rebuild
            TrimPool();
        ...
        //Creates or reuses a GameObject, attaches the given mesh to it, and assigns its material
        private void CreateGeneratedMeshObject(string meshName, Mesh mesh, Material material)
        {
            GameObject meshObject = null;

            //Looks for an existing procedural mesh object in pool
            for (int i = 0; i < _editorObjectPool.Count; i++)
            {
                if (_editorObjectPool[i] != null && _editorObjectPool[i].name == meshName)
                {
                    meshObject = _editorObjectPool[i];
                    _editorObjectPool.RemoveAt(i); //pull it from the pool
                    break;
                }
            }
            ...
            if (meshObject == null)
            {
                //makes a new instance if not found in pool
                meshObject = new GameObject(meshName);
                meshObject.transform.SetParent(_contentContainer, false);
                meshObject.AddComponent<MeshFilter>();
                meshObject.AddComponent<MeshRenderer>();
            }
            else
            {
                //destroy the old mesh data attached to the reused object to prevent memory leaks
                DestroyGeneratedMesh(meshObject);
            }
        ...
        //Spawns a prefab by checking the pool first. If missing, it instantiates and strips the (Clone) tag.
        private GameObject Spawn(GameObject prefab, Vector3 position, Quaternion rotation)
        {
            GameObject spawnedObject = null;

            //look for an object in the pool that was instantiated from the same prefab source
            for (int i = 0; i < _editorObjectPool.Count; i++)
            {
                if (_editorObjectPool[i] != null && _editorObjectPool[i].name == prefab.name)
                {
                    spawnedObject = _editorObjectPool[i];
                    _editorObjectPool.RemoveAt(i);
                    break;
                }
            }
            ...
            {
                //reuses assets, just moves and rotates
                spawnedObject.transform.position = position;
                spawnedObject.transform.rotation = rotation;
            }
            else
            {
                //controls instances. Fallback to creating a new one if pool was empty
                spawnedObject = Instantiate(prefab, position, rotation, _contentContainer);
        ...
        {
            if (refs == null) refs = new ReferenceSettings();
            if (refs.plankPrefabs == null) refs.plankPrefabs = new List<GameObject>();
            if (planks == null) planks = new PlankSettings();
            if (stringers == null) stringers = new StringerSettings();
            if (ropeBridge == null) ropeBridge = new RopeBridgeSettings();
            if (woodenBridge == null) woodenBridge = new WoodenBridgeSettings();
            if (pillars == null) pillars = new PillarSettings();
        }

        //Gathers all generated child objects from the container into our pool list for reuse
        private void PopulatePool()
        {
            _editorObjectPool.Clear();
            
            Transform container = transform.Find("[Bridge Gen Content]");
            if (container == null) return;

            //move all current objects/children into the pool list
            for (int i = container.childCount - 1; i >= 0; i--)
            {
                GameObject child = container.GetChild(i).gameObject;
                _editorObjectPool.Add(child);
            }
        }

        //Safely destroys any leftover objects in the pool that were not needed for the current rebuild
        private void TrimPool()
        {
            for (int i = 0; i < _editorObjectPool.Count; i++)
            {
                GameObject leftover = _editorObjectPool[i];
                if (leftover == null) continue;

                //destroys the raw mesh to clear memory
                DestroyGeneratedMesh(leftover);

                //destroys leftover game object
